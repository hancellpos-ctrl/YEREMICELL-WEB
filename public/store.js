...
import {parseOrder,summarizeOrder,whatsappOrder} from './order.js';
...
function syncOrderButtons(){
 ...
 document.querySelectorAll('[data-add-order]').forEach(button=>{
  const p=state.products.find(p=>p.id===button.dataset.addOrder),added=selection.has(button.dataset.addOrder);
  button.disabled=!p||p.status!=='available';button.classList.toggle('in-order',added);
  button.innerHTML=button.disabled?'Agotado':added?`${icon('check')} En mi lista`:`${icon('plus')} Añadir`;
  button.setAttribute('aria-label',`${added?'Ver en mi lista':'Añadir a mi lista'}: ${p?.name||''} ${p?.storage||''}`);
 });
}
function reconcileOrder(){
 const available=new Set(state.products.filter(p=>p.status==='available').map(p=>p.id));let removed=0;
 for(const [id,quantity] of selection)if(!available.has(id)){selection.delete(id);removed+=quantity}
 if(removed){persistOrder();orderRemoved+=removed}return removed;
}
function addToOrder(id){
 if(selection.has(id)){openOrder();return}
 const p=state.products.find(p=>p.id===id);if(!p||p.status!=='available')return;
 selection.set(id,1);persistOrder();syncOrderButtons();toast('Añadido a mi lista');
}
...
function renderOrder(){
 ...
 $('#order-items').innerHTML=summary.items.length?summary.items.map(({product:p,quantity,lineCents})=>`<article class="order-item"><div class="order-photo">${p.images.length?`<img src="${esc(p.images[0])}" alt="" loading="lazy">`:icon('phone')}</div><div class="order-info"><h3>${esc(p.name)}</h3><p>${esc(p.storage)}${p.condition!=='Consultar'?' · '+esc(p.condition):''}</p><small>${money(p.price)}${p.price!==null?' c/u':''}</small></div><button type="button" class="icon-button order-remove" data-order-remove="${p.id}" aria-label="Quitar ${esc(p.name)} ${esc(p.storage)}">${icon('trash')}</button><div class="order-item-bottom"><div class="order-quantity"><button class="icon-button" type="button" data-order-quantity="${p.id}" data-delta="-1" aria-label="Restar una unidad de ${esc(p.name)}">−</button><span aria-label="Cantidad">${quantity}</span><button class="icon-button" type="button" data-order-quantity="${p.id}" data-delta="1" aria-label="Sumar una unidad de ${esc(p.name)}" ${quantity>=99?'disabled':''}>${icon('plus')}</button></div><strong class="order-line-total">${lineCents===null?'Por consultar':money(lineCents/100)}</strong></div></article>`).join(''):`<div class="order-empty">${icon('bag')}<h3>Tu lista está vacía</h3><p>Añade los teléfonos que te interesan.</p><button class="btn primary" type="button" id="browse-order">Ver teléfonos ${icon('arrow')}</button></div>`;
...
async function openOrder(){
 ...
 try{
  ...
  orderNotice(removed?`Se quitaron ${removed} ${removed===1?'artículo que ya no está disponible':'artículos que ya no están disponibles'}.`:!state.settings.whatsapp?'La tienda no tiene WhatsApp disponible. Puedes copiar tu lista.':'');
...
$('#order-items').addEventListener('click',e=>{
 ...
 else if(quantity){const id=quantity.dataset.orderQuantity,n=(selection.get(id)||0)+Number(quantity.dataset.delta);if(n<1)selection.delete(id);else selection.set(id,Math.min(n,99))}
 else return;
